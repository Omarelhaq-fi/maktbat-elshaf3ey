...
export const Route = createFileRoute("/login")({
  component: LoginPage,
  head: () => ({ meta: [{ title: "تسجيل الدخول — مكتبة الشافعي" }] }),
});
    ...
    <section className="mx-auto max-w-md px-4 py-14 sm:px-6">
      <div className="rounded-2xl border border-border/60 bg-card p-8 shadow-sm">
        ...
        <p className="mt-2 text-center text-sm text-muted-foreground">
          {mode === "login" ? "ادخل بريدك وكلمة السر" : "افتح حساب علشان تقدر تطلب"}
        </p>

        <form onSubmit={submit} className="mt-6 space-y-4">
          <div>
            <label className="block text-sm font-bold text-brand-dark mb-1">البريد الإلكتروني</label>
            <input
              type="email"
              required
              value={email}
          ...
          <div>
            <label className="block text-sm font-bold text-brand-dark mb-1">كلمة السر</label>
            <input
              type="password"
              required
              minLength={6}
        ...
        <button
          ...
          disabled={loading}
          className="mt-4 w-full flex items-center justify-center gap-3 rounded-full border border-input bg-background py-3 text-sm font-bold text-brand-dark hover:bg-muted transition-colors disabled:opacity-50"
        >
        ...
        <button
          ...
          className="mt-4 w-full text-center text-sm text-brand-dark hover:text-brand-orange"
        >
          {mode === "login" ? "معندكش حساب؟ اعمل واحد" : "عندك حساب؟ سجل دخول"}
        </button>
      </div>
    </section>
  );
